...
def inference(images, labels, batch_size, phase_train):
    ...
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                           padding='SAME', data_format='NHWC', name='pool1')
    # conv2
    conv2 = conv_layer_with_bn(pool1, [7, 7, 64, 64], phase_train, name="conv2")

    # pool2
    pool2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1],
                           strides=[1, 2, 2, 1], padding='SAME', data_format='NHWC', name='pool2')
    # conv3
    conv3 = conv_layer_with_bn(pool2, [7, 7, 64, 64], phase_train, name="conv3")

    # pool3
    pool3 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1],
                           strides=[1, 2, 2, 1], padding='SAME', data_format='NHWC', name='pool3')
    # conv4
    conv4 = conv_layer_with_bn(pool3, [7, 7, 64, 64], phase_train, name="conv4")

    # pool4
    pool4 = tf.nn.max_pool(conv4, ksize=[1, 2, 2, 1],
                           strides=[1, 2, 2, 1], padding='SAME', data_format='NHWC', name='pool4')
    """ End of encoder """
    """ start upsample """
    # upsample4
...
def training(FLAGS, is_finetune=False):
  ...
  with tf.Graph().as_default():
    ...
    # For CamVid
    print(label_filenames, len(label_filenames))
    images, labels = CamVidInputs(image_filenames, label_filenames, batch_size)
    print(val_label_filenames, len(val_label_filenames))
    ...
    print(startstep, (startstep + max_steps))
    with tf.Session() as sess:
