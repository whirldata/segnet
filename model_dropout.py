...
def inference_dropout(images, labels, batch_size, phase_train,keep_prob):
    print("---------Dropout----------")
    ...
    # conv2
    conv2 = conv_layer_with_bn(dropout1, [7, 7, 64, 64], phase_train, name="conv2")
    ...
    upsample3= deconv_layer(decode_dropout3, [2, 2, 64, 64], [batch_size, 90, 120, 64], 2, "up3")
    # decode 3
    conv_decode3 = conv_layer_with_bn(upsample3, [7, 7, 64, 64], phase_train, False, name="conv_decode3")
    ...
    upsample2= deconv_layer(decode_dropout2, [2, 2, 64, 64], [batch_size, 180, 240, 64], 2, "up2")
    # decode 2
    conv_decode2 = conv_layer_with_bn(upsample2, [7, 7, 64, 64], phase_train, False, name="conv_decode2")
    ...
    upsample1= deconv_layer(decode_dropout1, [2, 2, 64, 64], [batch_size, 360, 480, 64], 2, "up1")
    # decode4
    conv_decode1 = conv_layer_with_bn(upsample1, [7, 7, 64, 64], phase_train, False, name="conv_decode1")
    """ end of Decode """
    """ Start Classify """
...
def test(FLAGS):
  ...
  #loss, logits = inference_dropout(test_data_node, test_labels_node, batch_size, phase_train,keep_prob)
  ...
  with tf.Session() as sess:
    ...
    while(True):
      ...
      cv2.imwrite('body-test.png',frame)
      im = np.array(skimage.io.imread('body-test.png'), np.float32)
      test_me = frame.reshape(1,360,480,3)
      #print(frame)
      #frame = tf.expand_dims(frame,0)
      images.append(frame)
      feed_dict = {
        test_data_node: frame.reshape(1,360,480,3),
        test_labels_node: labels[0],
        phase_train: False
      }
      dense_prediction, im = sess.run([logits, pred], feed_dict=feed_dict)
      predict_img = writeImage(im[0])
      scipy.misc.imshow(predict_img)
      #cv2.imshow('Output',predict_img)
...
def training(FLAGS, is_finetune=False):
  ...
  with tf.Graph().as_default():
    ...
    loss, eval_prediction = inference_dropout(train_data_node, train_labels_node, batch_size, phase_train,keep_prob)
